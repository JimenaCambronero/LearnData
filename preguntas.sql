...
SELECT COUNT(distinct (id_pedido)), COUNT(*) 
FROM pedido_articulo;
...
SELECT monthname(fecha_creacion), 
AVG(precio) AS media_precio, 
AVG(costo) AS media_coste 
FROM pedidos;

SELECT MONTH(fecha_creacion), AVG(precio) AS media_precio, 
AVG(costo) AS media_coste FROM pedidos
WHERE YEAR(fecha_creacion)=2013
...
AVG(costo) AS media_coste FROM pedidos
WHERE YEAR(fecha_creacion)=2013
GROUP BY MONTHNAME(fecha_creacion), 
MONTH(fecha_creacion) 
ORDER BY 2 ASC;
...
SELECT YEAR(FECHA_CREACION) AS ANYO ,
ROUND(SUM(PRECIO)/1000,2) AS VENTAS_X_MIL_EUR,
TRUNCATE(SUM(PRECIO)/1000,2) AS VENTAS_X_MIL_EUR_TRUNC
FROM PEDIDOS
GROUP BY YEAR(FECHA_CREACION) 
ORDER BY 1 DESC;
...
ROUND(AVG(PRECIO),2) AS AVG_VENTAS 
FROM PEDIDOS
GROUP BY ANYO, MES
...
MONTH(FECHA_CREACION) AS MES, 
ROUND(AVG(PRECIO),2) AS AVG_VENTAS,
AVG(ARTICULOS_COMPRADO) AS MEDIA_ART_PEDIDO 
FROM PEDIDOS
GROUP BY ANYO, MES
ORDER BY 1 DESC, 2 DESC;
...
SELECT * FROM PEDIDOS 
WHERE ID_PEDIDO = 7300;

SELECT * FROM PEDIDO_ARTICULO 
WHERE ID_PEDIDO = 7300;

SELECT ID_PEDIDO, COUNT(*), COUNT(DISTINCT ID_PRODUCTO) 
FROM pedido_articulo 
GROUP BY id_pedido;

#Ordenando por el importe total recaudado por producto, nos aparece el producto 1. 
SELECT ID_PRODUCTO,SUM(PRECIO) AS IMP_TOTAL_PROD 
FROM PEDIDO_ARTICULO 
...
SELECT DISTINCT ID_PRODUCTO,PRECIO-COSTO AS MARGEN_BRUTO 
FROM PEDIDO_ARTICULO;

#Lo podemos hacer agrupando (aunque el valor es el mismo para todo). Query mas liosa y con mas probabilidad de error.
SELECT ID_PRODUCTO,SUM(PRECIO)/COUNT(*)-SUM(COSTO)/COUNT(*) AS MARGEN_BRUTO 
FROM PEDIDO_ARTICULO 
GROUP BY ID_PRODUCTO;
...
SELECT PA.ID_PRODUCTO,PROD.NOMBRE_PRODUCTO,MIN(PA.FECHA_CREACION) AS PRIMERO_PEDIDO 
...
SELECT PA.ID_PRODUCTO,PROD.NOMBRE_PRODUCTO,MIN(PA.FECHA_CREACION)  AS PRIMERO_PEDIDO , 
PROD.FECHA_CREACION AS FECHA_LANZAMIENTO,(MIN(PA.FECHA_CREACION) - PROD.FECHA_CREACION) AS TIEMPO_DSD_LANZ_VENT 
FROM PEDIDO_ARTICULO PA
LEFT JOIN PRODUCTOS PROD
ON PROD.ID_PRODUCTO = PA.ID_PRODUCTO
GROUP BY PROD.NOMBRE_PRODUCTO,PA.ID_PRODUCTO;
...
SELECT YEAR(FECHA_CREACION) AS ANYO, 
SUM(PRECIO) AS VENTAS, SUM(PRECIO-COSTO) AS MARGEN 
FROM PEDIDO_ARTICULO 
GROUP BY ANYO;

#Adicionalmente, calculamos el % que supone cada producto sobre el total. 
SELECT ID_PRODUCTO,(SUM(PRECIO)/(SELECT SUM(PRECIO) 
FROM PEDIDO_ARTICULO))*100 AS PCT 
FROM PEDIDO_ARTICULO 
GROUP BY ID_PRODUCTO;

#Validamos el resultado.
SELECT ID_PRODUCTO,SUM(PRECIO)/1938509.75*100 
FROM PEDIDO_ARTICULO 
GROUP BY ID_PRODUCTO;
